...
from baseline import DefensiveReflexAgent
...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'DefensiveReflexAgent', second = 'AstarAttacker'):
...
class AstarAttacker(DummyAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if len(self.safeFoods) < 1 and len(self.getCapsules(gameState)) != 0 and self.opponentscaredTime(gameState) is not None and self.opponentscaredTime(gameState) < 10:
